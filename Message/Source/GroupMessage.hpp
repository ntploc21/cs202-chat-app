...
public:
    GroupMessage();
    GroupMessage(int group_id, int conversation_id, std::string group_name,
    ...
    friend void operator>>(const YAML::Node& in, GroupMessage& group_message);

public:
    std::optional< Message > send_message(int sender_id, Message message) const;
    std::optional< Message > send_message(int sender_id,
